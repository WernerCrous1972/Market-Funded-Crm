...
class SyncWithdrawalsJob implements ShouldQueue
{
    ...
    {
        Log::info('SyncWithdrawalsJob: starting', ['since' => $this->since]);
        ...
        // See SyncDepositsJob for the rationale — MTR snapshots manager-at-time
        // historically; we capture it per transaction row.
        ...
        foreach ($mtr->allWithdrawals($this->since) as $raw) {
            $stats['total']++;
            ...
            try {
                ...
                // ── Lead source filter (exclude DISTRIBUTOR/STAFF) ─────────
                ...
                // ── Upsert trading account from withdrawal data ────────────
                ...
                if ($this->dryRun) {
                    Log::info("DRY-RUN withdrawal: {$email} amount={$amountCents}");
                    $stats['created']++;
                    continue;
                }
                ...
                // Per-transaction historical account manager (see deposits sync).
                ...
                Activity::record(
                    $person->id,
                    'WITHDRAWAL',
                    "Withdrawal of \${$amountUsd} via " . ($gatewayInfo['name'] ?? 'unknown'),
                    [
                        'amount_cents'   => $amountCents,
                ...
                $stats['errors']++;
                Log::error('SyncWithdrawalsJob: error', [
        ...
        Log::info('SyncWithdrawalsJob: complete', $stats);
    }
}
